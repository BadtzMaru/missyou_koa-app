...
router.post(
	...
	async (ctx) => {
		...
		if (profile.length > 0) {
			...
			const isLike =
				post.likes.filter(
					(like) => like.user.toString() === ctx.state.user.id
				).length > 0;
			if (isLike) {
				ctx.status = 400;
				ctx.body = { alreadliked: '该用户已赞过' };
				return;
			}
...
/*
    @route POST api/posts/comment?id=xxx&comment_id=xxx
    @desc 删除评论
    @access 接口是私有的
*/
router.delete(
